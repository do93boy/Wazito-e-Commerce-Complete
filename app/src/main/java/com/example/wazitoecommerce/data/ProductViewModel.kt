...
class ProductViewModel(var navController: NavController, var context:Context) {
    ...
    fun allProducts(product:MutableState<Product>, products:SnapshotStateList<Product>):SnapshotStateList<Product>{
        progress.show()
        ...
        ref.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (snap in snapshot.children){
                    var retrieveProduct = snap.getValue(Product::class.java)
                    product.value = retrieveProduct!!
                    products.add(retrieveProduct)
                }
                progress.dismiss()
            }
